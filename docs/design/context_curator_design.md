...
> W4 Memory Promotion 由 [M23](../plans/m23-memory-promotion-stage-w4.md)
> 承载（2026-09-22 立项，契约语义随其 §4 跑前冻结）；
> W5 进入里程碑的门槛见 §13）
> 版本：0.2
> 更新日期：2026-09-15
...
| W4 | Memory Promotion：Curator 产生 Memory candidate，仍经 `MemoryConsolidator` 既有纪律——由 [M23](../plans/m23-memory-promotion-stage-w4.md) 承载（2026-09-22 立项并跑前冻结：`consolidate_candidates` 共享管线入口、section→kind 冻结映射、`Unverified`+`model_assisted` 纪律、duplicate 判定收紧禁止验证等级降级、宿主显式触发经泛型 Deferrable 路由；边界测试随其 §6 冻结） | W2 关闭（已满足）；Working Context 与长期 Memory 边界测试冻结（已满足，M23 §6 跑前冻结） |

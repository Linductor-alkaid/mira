...
- MCP 式运行时动态发现与热插拔：与“运行中不能由模型新增 native Tool”的安全立场冲突；
  ...
  （2026-09-15 注记：「部署与初始化时注册的 MCP Tool 来源」已由
  [DEC-039](DEC-039-mcp-tool-module-admission.md) 部分解除否决并以 ToolModule 身份
  纳入本决策信任域；「运行中动态发现与热插拔」的否决维持不变。本决策其余条款不变。）
...
- 模组冲突解析首期保守（后注册者被拒绝并记录），可能要求宿主调整模组划分而不是由
  Runtime 自动仲裁。
  （2026-09-19 注记：M7 TM2 冻结 wire 命名规则 v1——扁平命名空间、成员名原文进入、
  跨模组唯一性由协商/激活 fail-closed 门禁承载、hosted 保留名整组排除、命名空间前缀
  仅为非约束性约定；不设全局命名机构。详见
  [工具模组设计](../design/tool_module_design.md) §17 收口与
  [M7](../plans/m7-tools-evaluation-platform-v1.md) `M7-TM2-02`。）

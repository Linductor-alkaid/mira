...
> 状态：Accepted（方向冻结；全部三个实现阶段已于 2026-09-22 前后依
> [Tool 稳定引用与 Skill 设计](../design/tool_reference_and_skill_design.md)
> 冻结细项并由 [M7](../plans/m7-tools-evaluation-platform-v1.md) 交付关闭：
> TR0——稳定引用与兼容投影（PR #63，合并提交 `a180e88`）；TR1——Skill
> 发布生命周期与 Procedure 索引投影（PR #64，合并提交 `2833bc4`）；TR2——
> Runtime 接线与执行（IR v1.1 引用表达、库挂载、`create_run` 准入消费、
> `Degraded` 事件、Skill 经 Tool 通道子 Workflow 执行、Procedure 写入
> 接线）。引用语法 v1 与 Skill 契约随阶段冻结）
> 日期：2026-09-15
...
- [Tool 稳定引用与 Skill 设计](../design/tool_reference_and_skill_design.md)
  （TR0–TR2 阶段规范：§4–§13 引用语法 v1、提取、解析矩阵与兼容投影；§17
  Skill 生命周期与 Procedure 索引投影；§18 Runtime 接线与执行）
- [M7 计划](../plans/m7-tools-evaluation-platform-v1.md)（各阶段工作项、
  门禁与验证记录；引用语法冻结是 TR0 的退出条件之一，TR1/TR2 细项随其立项
  冻结）。

...
> [M12](m12-app-model-and-navigation.md)（实现已交付；Android 验收缺口已由 `MNT-202609-22` 补齐；阶段 F 的直接前置是阶段 D/E）
...
2026-09-09：阶段 F 后状态审计重新打开 `M13-09` 与跨平台退出条件，
...
`workflow_learning.cpp` 域映射、Episode/Lesson 与失败检索实现，NDK 26.3.11579264、
API 24、warnings-as-errors）并完成 `Mira::workflow` 安装闭包链接；本机复现细节见
M8 同日记录。Android 设备运行、宿主消费与跨重启学习持久化仍分别由
`MNT-202609-27/25` 跟踪，未验证不声明。`M13-09` 与重开门禁退出条件据此复核关闭，
里程碑恢复 `Completed`。

2026-09-09：`MNT-202609-25` 学习持久化取证落地（证据与差异清单见
[后续计划](maintenance-2026-09-post-stage-f.md) 第 5 节同日记录）：真实
`SqliteMemoryStore` 写入 Episode/Lesson 后关闭并重建整个 owner 栈，同 scope 逐字段
一致、跨 scope 不可读、确定性 mutation id 幂等重放；慢/失败 store 下升级、取消与
shutdown 均闭合。DEC-030 §5 事件重建配方逐字段取证未通过：载荷缺口登记

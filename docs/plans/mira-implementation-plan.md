...
| [M3](m3-model-provider-agent-loop.md) | OpenAI-compatible Provider 和视觉离散闭环 | M2 | Agent loop alpha | In Progress（实现与跨平台传输已验收；仅 interop 开放） |
...
文档。`Planned` 仅表示范围和验收方式已明确，不表示前置已满足或实现已开始。M3 的跨平台 TLS
目标证据已由 `M3-04` 关闭；当前只剩真实 Provider 互操作由 `M3-19` 原编号跟踪，后续里程碑不得
通过复制或改号绕过 M3 退出条件。

...
> 更新日期：2026-09-22（DEC-035 下一阶段 Stage W4「Memory Promotion」经
...
| [M23](m23-memory-promotion-stage-w4.md) | Context Curator Stage W4——Memory Promotion（[DEC-035](../decisions/DEC-035-context-curator-working-context.md)：快照耐久语句经 `MemoryConsolidator` 既有纪律晋升长期记忆——`consolidate_candidates` 共享管线入口、section→kind 冻结映射、`Unverified`+`model_assisted` 纪律、duplicate 判定收紧禁止验证等级降级；宿主显式触发、泛型 Deferrable 路由；无模型） | M21（Stage W2 关闭）；[Context Curator 设计](../design/context_curator_design.md) §13 与 M23 §4 冻结 | Stage W5 subagent fork/merge 与宿主集成轮的输入形态锚点（非发布物） | In Progress |
...
16. DEC-035 Stage W4「Memory Promotion」立项与冻结入口（2026-09-22）：M7
    ...
    测试由 Independent-Verification-Agent 独立取证；交付与关闭证据随
    [M23 文件](m23-memory-promotion-stage-w4.md) 验证记录回填。

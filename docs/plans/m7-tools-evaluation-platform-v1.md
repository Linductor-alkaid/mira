...
2026-09-09 跟进入口：[阶段 F 后续计划](maintenance-2026-09-post-stage-f.md)
`MNT-202609-27/28/30` 汇总真实需求、最小评估 profile 与 M7 任务迁移提案。本文件的
范围、旧前置和工作项继续冻结；解除 Blocked 仍需 DEC-011 要求的证据与范围决策。
M8–M13 已合入的实现可作为提案输入，其 Android 验收缺口已由 `MNT-202609-22` 补齐
（编译与安装闭包链接证据），设备运行证据仍归 `MNT-202609-27`。

...
[[nodiscard]] WorkflowDefinition induction_definition() {
    ...
    definition.steps = {parameterized_tool_step("mode"), tool_step("channeler", std::nullopt)};
    return definition;
}
...
int structural_diff_names_varying_leaves_only() {
    ...
    const auto candidates = induce_parameters({fast_trajectory.value(), slow_trajectory.value()});
    ...
    const auto constants = induce_parameters({fast_trajectory.value(), again_trajectory.value()});
...
int parameterized_compilation_reopens_parameters() {
    ...
    MIRA_CHECK(reference != nullptr && reference->is_string() && *reference->as_string() == "mode");
    ...
    const auto defaulted =
        bind_workflow_parameters(compiled.value(), JsonValue{JsonValue::Object{}});
    MIRA_CHECK(defaulted.has_value());
    const auto resolved_default =
        resolve_step_arguments(defaulted.value(), compiled.value().steps.front().arguments);
    ...
    const auto resolved_override =
        resolve_step_arguments(overridden.value(), compiled.value().steps.front().arguments);
...
int explicit_candidates_validate_fail_closed() {
    ...
    const auto expect_failure = [&](WorkflowParameterCandidate bad, WorkflowCompileError code) {
        const auto outcome = compile_workflow(trajectory.value(), options, {bad});
        if (outcome.has_value() || outcome.error().domain_code != static_cast<std::int32_t>(code)) {
...
int induction_rejects_mismatched_skeletons_and_types() {
    ...
    reshaped.steps.front().arguments.set("payload",
                                         JsonValue{JsonValue::Object{{"nested", JsonValue{"x"}}}});
...
int end_to_end_patch_repair_compiles_into_new_version() {
    ...
    auto step = tool_step(
        "counter", parameter_predicate("mode", WorkflowPredicateOp::Eq, JsonValue{"repaired"}));
    ...
    const auto repaired = workflow->patch_run(created.value().run_id, WorkflowPatchId::generate(),
    ...
    const auto published = workflow->publish_validated(compiled.value(), "m11-test",
                                                       "bake the repair", created.value().run_id);
...
int end_to_end_induced_version_reuses_parameters() {
    ...
    const auto default_run =
        workflow->create_run(compiled.value().workflow_id, published.value().ir_digest,
                             JsonValue{JsonValue::Object{}}, WorkflowPolicy::Strict);
    MIRA_CHECK(default_run.has_value());
    const auto default_driven = workflow->execute_run(default_run.value().run_id, drive_context());
    ...
    const auto override_run = workflow->create_run(
        ...
        JsonValue{JsonValue::Object{{"mode", JsonValue{"turbo"}}}}, WorkflowPolicy::Strict);
    ...
    MIRA_CHECK(channeler.dispatches.load() ==
               8); // 2 + 2 + 2 + 2 dispatches, gates dispatch nothing
    return 0;
}
...
int main() {
    const auto scenarios = std::to_array(
        {structural_diff_names_varying_leaves_only, parameterized_compilation_reopens_parameters,
         explicit_candidates_validate_fail_closed, induction_rejects_mismatched_skeletons_and_types,

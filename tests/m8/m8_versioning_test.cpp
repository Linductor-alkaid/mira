...
[[nodiscard]] WorkflowVersionRecord
make_record(SemanticVersion version, Sha256Digest content_digest, Sha256Digest parent_digest,
            WorkflowValidationResult validation, bool with_evidence = true) {
...
int history_is_append_only_and_chained() {
    ...
    MIRA_CHECK(append_workflow_version(history, make_record({2, 0, 0}, v1, digest_string("nope"),
                                                            WorkflowValidationResult::DryRunPassed))
                   .has_value() == false);
    MIRA_CHECK(append_workflow_version(history, make_record({1, 0, 0}, v1, Sha256Digest{},
                                                            WorkflowValidationResult::DryRunPassed))
                   .has_value());
    ...
    MIRA_CHECK(!append_workflow_version(
                    ...
                    .has_value());
    MIRA_CHECK(
        !append_workflow_version(history, make_record({2, 0, 0}, v2, digest_string("wrong-parent"),
    ...
    MIRA_CHECK(
        !append_workflow_version(history, make_record({3, 0, 0}, digest_string("v3"), v2,
    ...
    MIRA_CHECK(
        !append_workflow_version(orphan, make_record({1, 0, 0}, digest_string("o"), Sha256Digest{},
...
int only_validated_versions_are_runnable() {
    ...
    history.workflow_id = WorkflowId::generate();
    MIRA_CHECK(append_workflow_version(
                   history, make_record({1, 0, 0}, digest_string("draft"), Sha256Digest{},
                                        WorkflowValidationResult::NotValidated, false))
                   .has_value());
    MIRA_CHECK(!latest_runnable_workflow_version(history).has_value());

    MIRA_CHECK(append_workflow_version(history, make_record({2, 0, 0}, digest_string("rejected"),
                                                            digest_string("draft"),
    ...
    MIRA_CHECK(
        append_workflow_version(history, make_record({3, 0, 0}, digest_string("dry-run-passed"),
    ...
    MIRA_CHECK(
        append_workflow_version(history, make_record({4, 0, 0}, digest_string("fully-validated"),
    ...
    MIRA_CHECK(
        append_workflow_version(history, make_record({5, 0, 0}, digest_string("wip"),
                                                     digest_string("fully-validated"),
                                                     WorkflowValidationResult::NotValidated, false))
            .has_value());

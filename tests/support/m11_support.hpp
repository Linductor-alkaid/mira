...
struct ChannelTool final {
    ...
    [[nodiscard]] BuiltinToolRegistration registration(const std::string &wire_name = "channeler") {
...
[[nodiscard]] inline WorkflowStep
parameterized_tool_step(const std::string &parameter, const std::string &wire = "channeler",
                        bool side_effect_tool = false,
    ...
    arguments.emplace_back("payload",
                           JsonValue{JsonValue::Object{{"$param", JsonValue{parameter}}}});
...
[[nodiscard]] inline WorkflowDefinition
compilable_definition(const std::vector<WorkflowStep> &steps, std::string name = "m11-flow",

...
std::string canonical(const EventEnvelope &event) {
    ...
    if (event.task_id) stream << event.task_id->to_string();
    stream << '|' << event.session_sequence << '|';
    if (event.task_sequence) stream << *event.task_sequence;
...
MemoryEventStore::MemoryEventStore(std::size_t max_events) : impl_(std::make_unique<Impl>(max_events)) {}
...
Result<AppendReceipt> MemoryEventStore::append(const AppendRequest &request) {
    ...
    if (request.task_id) event.task_sequence = ++impl_->next_task[*request.task_id];
    event.cause_control_sequence = request.cause;
...
Result<std::vector<AppendReceipt>> MemoryEventStore::append_batch(std::span<const AppendRequest> requests) {
...
Result<EventPage> MemoryEventStore::read(const EventQuery &query) const {
    ...
    for (const auto &event : impl_->events) {
        if (event.session_id != query.session_id || event.session_sequence <= after) continue;
...
Result<StoreRecoveryReport> MemoryEventStore::recover(const RecoveryOptions &) {
    std::lock_guard lock(impl_->mutex);
    return StoreRecoveryReport{RecoveryStatus::Clean, impl_->events.size(), 0, "memory store is consistent"};
}
...
std::optional<EventEnvelope> MemoryEventStore::find(EventId id) const {
    ...
    return found == impl_->by_id.end() ? std::nullopt : std::optional<EventEnvelope>(impl_->events[found->second]);
}
...
Result<AppendReceipt> FileEventStore::append(const AppendRequest &request) {
    ...
    if (!impl_->loaded) {
        ...
        if (!recovery) return recovery.error();
    ...
    std::lock_guard lock(impl_->mutex);
    if (impl_->read_only) return error(ErrorCode::DataLoss, "event store is read-only after recovery");
    if (request.event_id.is_nil() || request.runtime_id.is_nil() || request.session_id.is_nil()) {
        return error(ErrorCode::InvalidArgument, "event, runtime, and session IDs must be non-nil");
    }
    ...
    if (existing != impl_->integrity_by_id.end()) {
        ...
        if (!event) return error(ErrorCode::DataLoss, "event index is inconsistent");
        return AppendReceipt{event->event_id, event->session_sequence, event->task_sequence, 0, request.required};
    }
    const auto session_sequence = impl_->next_session[request.session_id] + 1;
    std::optional<TaskSequence> task_sequence;
    if (request.task_id) task_sequence = impl_->next_task[*request.task_id] + 1;
    std::ofstream output(impl_->log_path, std::ios::app);
    if (!output) return error(ErrorCode::Unavailable, "event log cannot be opened", true);
    ...
    output.flush();
    if (!output) return error(ErrorCode::Unavailable, "event log write failed", true);
    auto result = impl_->memory.append(request);
    if (!result || result.value().session_sequence != session_sequence || result.value().task_sequence != task_sequence) {
    ...
    if (request.task_id && task_sequence) impl_->next_task[*request.task_id] = task_sequence.value();
    impl_->integrity_by_id.emplace(request.event_id, requested_integrity);
    return result;
}

Result<std::vector<AppendReceipt>> FileEventStore::append_batch(std::span<const AppendRequest> requests) {
...
Result<StoreRecoveryReport> FileEventStore::recover(const RecoveryOptions &options) {
    ...
    if (impl_->loaded) {
        ...
        if (!memory_report) return memory_report.error();
        return StoreRecoveryReport{impl_->recovery_status, memory_report.value().events_recovered, 0,
    ...
    while (std::getline(input, line)) {
        ...
        if (!(stream >> sequence >> event_text >> runtime_text >> session_text >> task_text >> cause >>
              classification >> std::quoted(type) >> std::quoted(data))) {
            ...
            if (has_more || !options.repair_uncommitted_tail) {
                ...
                return StoreRecoveryReport{RecoveryStatus::ReadOnlyDiagnostic, recovered, line.size(),
                                            impl_->recovery_diagnostic};
            }
            ...
            if (size_error || file_size < last_good_offset) {
                ...
                return StoreRecoveryReport{RecoveryStatus::ReadOnlyDiagnostic, recovered, line.size(),
                                           "event log size could not be determined"};
            }
        ...
        AppendRequest request{*event_id, *runtime_id, *session_id, std::nullopt,
                              EventPayload{type, data, event_class.value()}, cause,
        ...
        if (task_text != "-") {
            ...
            if (!task_id) {
                ...
                return StoreRecoveryReport{RecoveryStatus::DataLoss, recovered, line.size(), "invalid task ID"};
            }
            request.task_id = *task_id;
        }
        if (!(stream >> integrity_text) || integrity_text != digest_string(canonical(request)).to_string()) {
            impl_->read_only = true;
            impl_->recovery_status = RecoveryStatus::DataLoss;
...
Result<void> FileEventStore::flush(Durability) {
    ...
    if (!output) return error(ErrorCode::Unavailable, "event log cannot be opened", true);
    output.flush();
    return output ? Result<void>{} : Result<void>(error(ErrorCode::Unavailable, "event log flush failed", true));
}

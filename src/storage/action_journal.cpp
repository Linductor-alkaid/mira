...
std::string intent_data(const ActionIntent &intent, std::string_view suffix = {}) {
    ...
    data << intent.action_id.to_string() << '|' << intent.task_id.to_string() << '|'
         << intent.task_epoch << '|' << intent.environment_epoch << '|'
         << intent.action_digest.to_string() << '|' << intent.target.type << '|' << intent.target.id
         << '|' << intent.target.scope;
    if (!suffix.empty())
        data << '|' << suffix;
    return data.str();
...
Result<AppendReceipt> ActionJournal::append(const ActionIntent &intent, std::string type,
    ...
    return store_.append({EventId::generate(), runtime_id_, intent.session_id, intent.task_id,
                          EventPayload{std::move(type), std::move(data), classification}, 0,
                          durability});
}
...
Result<AppendReceipt> ActionJournal::dispatch_started(const ActionIntent &intent) {
    ...
    const auto found = states_.find(intent.action_id);
    if (found == states_.end())
        return journal_error(ErrorCode::InvalidState, "action must be prepared first");
...
Result<AppendReceipt> ActionJournal::receipt(const ActionIntent &intent,
    ...
    if (found->second.phase == ActionJournalPhase::Receipt)
        return found->second.last_receipt.value();
    auto result = append(intent, "ActionReceipt", intent_data(intent, execution.safe_message),
                         EventClass::State, Durability::ProcessCrash);
    if (!result)
        return result.error();
    found->second.phase = ActionJournalPhase::Receipt;
    found->second.last_receipt = result.value();
...
Result<AppendReceipt> ActionJournal::execution_uncertain(const ActionIntent &intent,
                                                         std::string reason) {
    std::lock_guard lock(mutex_);
    const auto found = states_.find(intent.action_id);
    if (found == states_.end() || found->second.phase == ActionJournalPhase::Prepared) {
    ...
    if (found->second.phase == ActionJournalPhase::ExecutionUncertain)
        return found->second.last_receipt.value();
    auto result = append(intent, "ActionExecutionUncertain", intent_data(intent, reason),
                         EventClass::Critical, Durability::ProcessCrash);
    if (!result)
        return result.error();
    found->second.phase = ActionJournalPhase::ExecutionUncertain;
    found->second.last_receipt = result.value();
...
Result<ActionJournalState> ActionJournal::recover(const ActionIntent &intent) const {
    ...
    for (const auto &event : page.value().events) {
        if (event.task_id != intent.task_id || event.payload.data.rfind(marker, 0) != 0)
            continue;
        if (event.payload.type == "ActionDispatchStarted")
            state.phase = ActionJournalPhase::DispatchStarted;
        if (event.payload.type == "ActionReceipt")
            state.phase = ActionJournalPhase::Receipt;
        if (event.payload.type == "ActionExecutionUncertain")
            state.phase = ActionJournalPhase::ExecutionUncertain;
        state.last_receipt = AppendReceipt{event.event_id, event.session_sequence,
                                           event.task_sequence, 0, Durability::ProcessCrash};
    }
    return state;
}

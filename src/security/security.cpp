...
std::string effect_canonical(const ProposedEffect &effect, const ResourceDescriptor &target) {
    return effect.action + "|" + effect.parameters + "|" + std::to_string(static_cast<int>(effect.risk)) +
...
bool has_capability(const PrincipalContext &principal, std::string_view capability,
    ...
    return std::any_of(principal.grants.begin(), principal.grants.end(), [&](const auto &grant) {
        if (grant.capability != capability || (grant.expires_at != std::chrono::system_clock::time_point{} &&
                                                grant.expires_at <= now)) {
            return false;
        }
        if (!grant.resources.type.empty() && grant.resources.type != target.type) return false;
        if (!grant.resources.scope.empty() && grant.resources.scope != target.scope) return false;
...
PolicyDecision PolicyEngine::evaluate(const PolicyInput &input) const {
    ...
    if (input.effect.risk == ActionRisk::R4Critical) {
        return DenyDecision{version_, "critical-default-deny", "critical actions require explicit host policy"};
    }
    if (input.effect.risk == ActionRisk::R3Sensitive || input.effect.risk == ActionRisk::R2UserVisible ||
        input.sensitivity >= Sensitivity::Sensitive) {
        return RequireConfirmationDecision{version_, "confirmation-required", "user confirmation is required"};
    }
...
Result<ConfirmationChallenge> ConfirmationAuthority::issue(
    const PrincipalContext &principal, SessionId session_id, TaskId task_id, std::uint64_t task_epoch,
    std::uint64_t environment_epoch, const ProposedEffect &effect, const ResourceDescriptor &target,
    PolicyVersion policy_version, std::chrono::seconds lifetime) {
...
Result<void> ConfirmationAuthority::consume(
    const ConfirmationChallenge &challenge, const ConfirmationResponse &response,
    const PrincipalContext &principal, const ProposedEffect &effect, const ResourceDescriptor &target,
    std::uint64_t task_epoch, std::uint64_t environment_epoch, PolicyVersion policy_version) {
    std::lock_guard lock(mutex_);
    const auto found = std::find_if(states_.begin(), states_.end(), [&](const auto &state) {
        return state.challenge.id == challenge.id;
    });
    if (found == states_.end()) return error(ErrorCode::NotFound, "confirmation challenge is unknown");
    if (found->consumed) return error(ErrorCode::AlreadyExists, "confirmation challenge was already consumed");
    ...
    if (response.challenge_id != stored.id || response.nonce != stored.nonce ||
        ...
        stored.principal.tenant_id != principal.tenant_id || stored.principal.host_id != principal.host_id ||
        stored.session_id != challenge.session_id || stored.task_id != challenge.task_id) {
        return error(ErrorCode::PermissionDenied, "confirmation response identity or nonce mismatch");
    }
    if (response.decision != ConfirmationDecision::Approve) return error(ErrorCode::SafetyRejected, "confirmation rejected");
    if (std::chrono::system_clock::now() >= stored.expires_at) return error(ErrorCode::DeadlineExceeded, "confirmation expired");
    if (task_epoch != stored.task_epoch || environment_epoch != stored.environment_epoch ||
        policy_version != stored.policy_version || stored.action_digest != digest_string(effect_canonical(effect, target))) {
        return error(ErrorCode::SafetyRejected, "confirmation binding no longer matches the requested effect");
    }
    found->consumed = true;
    return Result<void>{};
}
...
bool ConfirmationAuthority::is_consumed(ConfirmationId id) const {
    std::lock_guard lock(mutex_);
    const auto found = std::find_if(states_.begin(), states_.end(), [&](const auto &state) { return state.challenge.id == id; });
    return found != states_.end() && found->consumed;
}

std::string Redactor::redact(std::string_view value, Sensitivity sensitivity) {
    if (sensitivity == Sensitivity::Public || sensitivity == Sensitivity::Internal) return std::string(value);
    if (value.empty()) return "<redacted>";
...
RedactionRecord Redactor::record(std::string_view value, Sensitivity sensitivity) {
    return RedactionRecord{sensitivity, value.size(), digest_string(value),
                           sensitivity == Sensitivity::Sensitive || sensitivity == Sensitivity::Secret};
}
...
bool private_ipv4(std::string_view host) {
    ...
    if (!(input >> octets[0] >> dot) || dot != '.') return false;
    if (!(input >> octets[1] >> dot) || dot != '.') return false;
    if (!(input >> octets[2] >> dot) || dot != '.') return false;
    if (!(input >> octets[3])) return false;
...
bool endpoint_allowed(std::string_view url, const EndpointPolicy &policy) {
    ...
    if (path_start != std::string_view::npos) authority = authority.substr(0, path_start);
    ...
    if (authority.front() == '[') {
        ...
        if (colon != std::string_view::npos) {
            ...
            if (port.empty() || !std::all_of(port.begin(), port.end(), [](unsigned char c) { return std::isdigit(c); })) return false;
        }
    }
    if (!policy.allow_private && (host == "localhost" || host == "::1" || private_ipv4(host) ||
                                  host == "metadata.google.internal" || host == "169.254.169.254")) return false;
    return std::any_of(policy.allowed_hosts.begin(), policy.allowed_hosts.end(), [&](const auto &allowed) {
...
bool path_within_root(const std::filesystem::path &root, const std::filesystem::path &candidate) {
    ...
    if (relative.empty() || relative == "." || relative == ".." || relative.is_absolute()) return false;
...
bool contains_prompt_injection(std::string_view value) {
    ...
    static constexpr std::string_view markers[] = {
        "ignore previous instructions", "ignore all instructions", "system message", "developer message",
        "jailbreak", "override the system", "reveal the prompt", "exfiltrate"};
    return std::any_of(std::begin(markers), std::end(markers), [&](auto marker) {
